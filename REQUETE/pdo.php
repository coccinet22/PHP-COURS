...
// rowCount()
echo '<b>Nombre de services dans la table employes : </b>' . $resultat->rowCount() . '<hr>';

// affichage dans un tableau html
$resultat = $pdo->query("SELECT DISTINCT service FROM employes");
echo '<style>th, td { padding: 10px; }</style>'; // du style css pour le tableau
echo '<table border="1" style="width: 20%; border-collapse: collapse;">';
echo '<tr style="background-color: #333; color: white;">';
echo '<th>Service</th>';
echo '</tr>';
...
echo '</table>';$resultat = $pdo->query("SELECT DISTINCT service FROM employes");
...
//Afficher la liste des bdd du serveur et le nbre
$res = $pdo->query("SHOW DATABASES ORDER BY databases");
// rowCount()
echo '<b>Nombre de bdd sur le serveur : </b>' . $res->rowCount() . '<hr>';while($bdd = $res->fetch(PDO::FETCH_ASSOC)) {
    //echo'<pre>';print-r($bdd)
    echo '<ul>';   
